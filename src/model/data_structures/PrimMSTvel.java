...
public class PrimMSTvel
{
    ...
    private void scan(Graph G, int v) {
        marked[v] = true;
        for (Arco e : (Iterable<Arco>)G.adj(v)){
            int w = e.other(v);
            if (marked[w]) continue;         // v-w is obsolete edge
            if (e.darCostoVelocidad() < distTo[w]) {
                distTo[w] = e.darCostoVelocidad();
                edgeTo[w] = e;
    ...
    private boolean check(Graph G) {
        ...
        for (Arco e : edges()) {
            totalWeight += e.darCostoVelocidad();
        }
